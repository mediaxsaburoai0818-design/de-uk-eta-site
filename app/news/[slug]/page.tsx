...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://eta-de.com/news/${n.slug}/` },
  };
}
    ...
    <div>
      ...
      <section className="py-12 md:py-16 bg-white">
        <article className="max-w-3xl mx-auto px-6">
          ...
          <div className="mt-10">
            <Link href="/news/" className="content-link font-bold">&larr; Alle aktuellen Informationen</Link>
          </div>

          <Breadcrumb items={[{ label: "Aktuelle Informationen", href: "/news/" }, { label: n.title }]} />
        </article>
      </section>
    </div>
  );
